...
class TestCommandHandlers:
    ...
    def test_format_list_of_posts(self, database, posts, expected_reply):
        ...
        assert list(format_list_of_posts(posts_orm))[0] == textwrap.dedent(
            expected_reply
        )
    ...
    @pytest.mark.parametrize(
        ...
        argvalues=[
            ...
            (("sale", 167791, "1012", "Jacob", "", 1), ": [Jacob](tg://user?id=1012)"),
        ],
        ids=["format_single_post", "error_format_single_post_without_game_name"],
    )
    ...
    async def test_disable_command(self, mock_update, update_details, mocker):
        ...
        if update_details["chat_type"] != "private":
            ...
            mock_disable_posts.assert_called_once_with(
                user_id=update_details["user_id"]
            )

    @pytest.mark.parametrize("chat_type", ("private", "group"))
    async def test_list_all_active_sales(
    ...
    async def test_list_my_active_posts(
        self, mock_update, update_details, mocker, database
    ):
        mock_update.effective_chat.type = update_details["chat_type"]
        mock_update.message.from_user.id = update_details["user_id"]
        # Create test posts
    ...
    @pytest.mark.parametrize(
        ["update_details", "initial_posts", "expected_output"],
        [
            (
                {
                    "chat_type": "group",
                ...
                [
                    {
                        ...
                        "user_name": "Jacob",
                        "post_type": "search",
                        "game": {"game_id": 1, "game_name": "Monopoly"},
                    },
                    {
                        "user_id": "102",
                        "user_name": "Henry",
                    ...
                    },
                    {
                        ...
                        "user_name": "Jacob",
                        "post_type": "search",
                        "game": {"game_id": 2, "game_name": "Ark Nova"},
                    },
                ],
                None,
            ),
    ...
    @pytest.mark.parametrize(
        ["update_details", "user_to_disable", "post_type"],
        [
            (
                {
                    "chat_type": "group",
    ...
    async def test_disable_user(
        self, mock_update, update_details, mocker, user_to_disable, post_type
    ):
        mock_update.effective_chat.type = update_details["chat_type"]
        mock_update.message.from_user.id = update_details["user_id"]
        mock_update.message.text = update_details["text"]
    ...
    @pytest.mark.parametrize(
        ["update_details", "user_to_disable", "game_id", "post_type"],
        [
            (
                {
                    "chat_type": "group",
    ...
    async def test_disable_post_for_user(
        self, update_details, user_to_disable, game_id, post_type, mock_update, mocker
    ):
        ...
        if (
            user_to_disable is None
            or post_type not in ["sale", "search"]
            or game_id is None
        ):
